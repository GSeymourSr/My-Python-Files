...
class CanvasButton(tk.Canvas):
    ...
    def _draw_shape(self, offset_x, offset_y, **kwargs):
        ...
        if shape == 'rounded_rect':
            points = []
            # This logic creates a smooth rounded rectangle polygon
            ...
            return self.create_polygon(points, **kwargs, smooth=False, outline="") # Smooth=False is more accurate with points
        elif shape == 'pill':
            radius = h / 2
...
class WhimsicalFileLauncherApp:
    ...
    def _build_ui(self):
        ...
        self.title_label = tk.Label(top_frame, text=self.current_menu_title, font=("Arial", 24, "bold"), bg=self.current_root_bg)
        ...
        appearance_frame = tk.LabelFrame(control_bar, text="Appearance & Layout", padx=5, pady=5)
        ...
        # --- CHANGE: Rearranged File Management widgets into a grid ---
        if not self.is_generated_menu:
            tk.Button(file_frame, text="🌟 Select Files", command=self.select_files_and_create_menu).grid(row=0, column=0, padx=2, pady=2, sticky='ew')
            tk.Button(file_frame, text="➕ Add Files", command=self.add_files_to_menu).grid(row=0, column=1, padx=2, pady=2, sticky='ew')
            tk.Button(file_frame, text="📂 Load Menu", command=self.load_menu_from_script).grid(row=0, column=2, padx=2, pady=2, sticky='ew')
            tk.Button(file_frame, text="✨ Merge Menu(s)", command=self.merge_menus_from_scripts).grid(row=0, column=3, padx=2, pady=2, sticky='ew') # NEW BUTTON
            tk.Button(file_frame, text="💾 Save Menu", command=self.prompt_and_save_menu, bg="#77DD77").grid(row=1, column=0, padx=2, pady=2, sticky='ew')
        
        tk.Button(file_frame, text="Select All", command=lambda: self.toggle_all_checkboxes(True)).grid(row=1, column=1, padx=2, pady=2, sticky='ew')
        tk.Button(file_frame, text="Select None", command=lambda: self.toggle_all_checkboxes(False)).grid(row=1, column=2, padx=2, pady=2, sticky='ew')
        
        for i in range(4): file_frame.grid_columnconfigure(i, weight=1) # Make columns resize equally

        # Appearance Widgets
        tk.Button(appearance_frame, text="🎨 Re-Enchant", command=lambda: self.create_menu_ui(preserve_styles=False)).grid(row=0, column=0, rowspan=2, padx=2, ipady=10)
        
        tk.Label(appearance_frame, text="Style Mode:").grid(row=0, column=1, sticky='w')
        tk.Radiobutton(appearance_frame, text="Whimsical", variable=self.style_mode_var, value="Whimsical").grid(row=0, column=2, sticky='w')
        tk.Radiobutton(appearance_frame, text="Uniform", variable=self.style_mode_var, value="Uniform").grid(row=0, column=3, sticky='w')
        
        tk.Label(appearance_frame, text="Theme:").grid(row=1, column=1, sticky='w')
        theme_options = ["Whimsical", "Monochromatic", "Analogous", "Complementary"] + sorted(list(PALETTES.keys())) # --- CHANGE: Sorted themes ---
        tk.OptionMenu(appearance_frame, self.theme_var, *theme_options).grid(row=1, column=2, columnspan=2, sticky='ew')

        tk.Label(appearance_frame, text="Sort By:").grid(row=0, column=4, padx=(10,0))
        ...
        tk.OptionMenu(appearance_frame, self.sort_var, *sort_options, command=self.sort_menu).grid(row=0, column=5, sticky='ew')

        tk.Label(appearance_frame, text="Cols:").grid(row=1, column=4, padx=(10,0))
        tk.Spinbox(appearance_frame, from_=1, to=20, width=3, textvariable=self.num_columns).grid(row=1, column=5)
    ...
    def create_menu_ui(self, preserve_styles=False):
        ...
        if not preserve_styles:
            ...
            for config in self.selected_file_configs:
                ...
                if saved_text:
                    config['style']['text'] = saved_text
        
        self._build_scrollable_frame()
        self._populate_menu_grid()

    def _reflow_menu_grid(self, *args):
        if not self.selected_file_configs or not self.menu_frame: return
    ...
    def sort_menu(self, *args):
        ...
        elif sort_key == "Date Modified (Newest)": key_func = lambda c: os.path.getmtime(c['path']); reverse = True
        elif sort_key == "Date Modified (Oldest)": key_func = lambda c: os.path.getmtime(c['path'])
        elif sort_key == "File Size (Largest)": key_func = lambda c: os.path.getsize(c['path']); reverse = True
        elif sort_key == "File Size (Smallest)": key_func = lambda c: os.path.getsize(c['path'])
        elif sort_key == "Group by Folder": key_func = lambda c: (os.path.dirname(c['path']), os.path.basename(c['path']).lower()) # Sort by name within folder
        else: return

        self.selected_file_configs.sort(key=key_func, reverse=reverse)
        if self.menu_frame: self._reflow_menu_grid()
    ...
    def _parse_menu_script(self, filename):
        """Helper to parse a menu script and return its data."""
        ...
        return {
            'title': get_value('SAVED_MENU_TITLE_STR'),
            'configs': get_value('SAVED_FILE_CONFIGS_DATA'),
            'bg': get_value('INITIAL_ROOT_BG_STR'),
            'settings': get_value('SAVED_APP_SETTINGS')
        }
    ...
    # --- CHANGE: New function to merge multiple menus ---
    def merge_menus_from_scripts(self):
        """Loads one or more menu scripts and adds their items to the current menu, skipping duplicates."""
            ...
            try:
                ...
                for config_data in data['configs']:
                    if config_data['path'] not in existing_paths:
                        ...
                        self.selected_file_configs.append(new_config)
                        existing_paths.add(new_config['path'])
                        added_count += 1
                    else:
                        skipped_count += 1
            except Exception as e:
                messagebox.showwarning("Merge Warning", f"Could not parse file:\n{os.path.basename(filename)}\nError: {e}\n\nSkipping this file.", parent=self.root)
