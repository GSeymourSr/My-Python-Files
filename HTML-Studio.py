...
try:
    ...
    print("MoviePy not found. Please run: pip install moviepy")
...
class AdvancedHTMLViewer:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Whimsical HTML Studio Pro - Fixed")
    ...
    def setup_gui(self):
        ...
        if MOVIEPY_AVAILABLE:
            self.btn_export = StyledButton(action_frame, text="💾 Export MP4 (Fixed)", bg="#b19cd9", fg="white", 
    ...
    def _process_mp4_robust(self, save_path):
        ...
        try:
            ...
            for idx, slide in enumerate(self.slides):
                ...
                # --- KEY FIX: Convert RGB and Resize ---
                clip = ImageClip(tmp_name).convert("RGB")
            ...
            self.root.after(0, lambda: messagebox.showerror("Export Failed", f"Error: {str(e)}"))
            ...
            self.root.after(0, lambda: self.toggle_controls(False))
            self.root.after(0, lambda: self.btn_export.config(text="💾 Export to MP4"))
