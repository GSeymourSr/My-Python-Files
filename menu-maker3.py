...
class WhimsicalFileLauncherApp:
    def __init__(self, root, is_generated_menu=False, initial_configs=None, initial_title="✨ Whimsical File Launcher Creator ✨", initial_root_bg=None):
        ...
        # --- GUI Setup ---
        # (This part is identical to your previous full version, so I'll omit for brevity here,
        # but it should be included in your actual file)
        # It sets up title_label_frame, title_label, top_control_frame,
        # left_controls (select_button, regenerate_button),
        # center_controls (multi_run_options_frame with delay, order, loop, run_selected),
        # right_controls (select_all, deselect_all, save_button),
        # and menu_display_area.
        # --- Start Full GUI Setup from previous version ---
    ...
    def _update_title_label_style(self):
        ...
        if hasattr(self, 'title_label') and self.title_label.winfo_exists(): bg_color = self._get_random_color();fg_color = self._get_contrasting_color(bg_color);font_family = random.choice(WHIMSICAL_FONTS);font_size = random.randint(18, 24) if self.is_generated_menu else random.randint(16, 20);self.title_label.config(text=self.current_menu_title,font=(font_family, font_size, "bold"),bg=bg_color, fg=fg_color)
        self.current_root_bg = self._get_random_color(light_bias=True);self.root.configure(bg=self.current_root_bg)
        if hasattr(self, 'menu_display_area') and self.menu_display_area.winfo_exists(): self.menu_display_area.configure(bg=self.current_root_bg)
        if hasattr(self, 'title_label_frame') and self.title_label_frame.winfo_exists(): self.title_label_frame.configure(bg=self.current_root_bg)
    ...
    def _get_random_color(self, light_bias=False, avoid=None):
        ...
        if light_bias:r,g,b=int(color[1:3],16),int(color[3:5],16),int(color[5:7],16);
        if(r+g+b)/3<128:
            for _ in range(3):nc=random.choice(WHIMSICAL_COLORS);nr,ng,nb=int(nc[1:3],16),int(nc[3:5],16),int(nc[5:7],16);
            if(nr+ng+nb)/3>=128:return nc
        return color

    def _get_contrasting_color(self, bg_color_hex):
        # (Identical to previous version)
        try:r,g,b=int(bg_color_hex[1:3],16),int(bg_color_hex[3:5],16),int(bg_color_hex[5:7],16);return"#000000"if(0.299*r+0.587*g+0.114*b)>128 else"#FFFFFF"
        except:return random.choice(["#000000","#FFFFFF"])

    def select_files_and_create_menu(self):
        # (Identical to previous version, ensures 'process_info': None is initialized)
        ...
        if files:
            ...
            no_files_label=tk.Label(self.menu_display_area,text="No files selected. The magic awaits!",font=("Comic Sans MS",18),bg=self.menu_display_area.cget("bg"),fg=self._get_contrasting_color(self.menu_display_area.cget("bg")));no_files_label.pack(expand=True)
    ...
    def create_menu_ui(self, preserve_styles=False):
        ...
        for widget in self.menu_display_area.winfo_children():widget.destroy()
        if not preserve_styles:self._update_title_label_style()
        menu_frame_bg=self._get_random_color(light_bias=True,avoid=self.current_root_bg);self.menu_frame=tk.Frame(self.menu_display_area,bg=menu_frame_bg);self.menu_frame.pack(expand=True,fill=tk.BOTH,padx=5,pady=5)
        canvas_bg=self._get_random_color(light_bias=True,avoid=menu_frame_bg);canvas=tk.Canvas(self.menu_frame,bg=canvas_bg,highlightthickness=0);scrollbar=tk.Scrollbar(self.menu_frame,orient="vertical",command=canvas.yview);self.scrollable_frame_in_canvas=tk.Frame(canvas,bg=canvas_bg)
        self.scrollable_frame_in_canvas.bind("<Configure>",lambda e:canvas.configure(scrollregion=canvas.bbox("all")));canvas.create_window((0,0),window=self.scrollable_frame_in_canvas,anchor="nw");canvas.configure(yscrollcommand=scrollbar.set)
        ...
        for i,config in enumerate(self.selected_file_configs):
            filepath=config['path'];filename=os.path.basename(filepath)
            if not preserve_styles or not config.get('style') or not config['style'].get('bg'):style={'bg':self._get_random_color(),'font_family':random.choice(WHIMSICAL_FONTS),'font_size':random.randint(10,14),'relief':random.choice(RELIEF_STYLES),'borderwidth':random.randint(1,3),'padx':random.randint(6,10),'pady':random.randint(3,6),'text':filename};style['fg']=self._get_contrasting_color(style['bg']);style['activebackground']=self._get_random_color(avoid=style['bg']);style['activeforeground']=self._get_contrasting_color(style['activebackground']);config['style']=style
            item_style=config['style'];actual_col=(i%NUM_COLUMNS)+1
            if i%NUM_COLUMNS==0 and i!=0: row+=1
            item_f=tk.Frame(self.scrollable_frame_in_canvas,bg=self.scrollable_frame_in_canvas.cget("bg"));item_f.grid(row=row,column=actual_col,padx=3,pady=3,sticky="ew")
            ...
            try: tk.Button(item_f,text=btn_txt,command=lambda p=filepath:self.launch_file(p),bg=item_style['bg'],fg=item_style['fg'],font=(item_style['font_family'],item_style['font_size']),relief=item_style['relief'],borderwidth=item_style['borderwidth'],padx=item_style['padx'],pady=item_style['pady'],activebackground=item_style['activebackground'],activeforeground=item_style['activeforeground'],width=12).pack(side=tk.LEFT,fill=tk.X,expand=True)
            except tk.TclError: tk.Button(item_f,text=btn_txt,command=lambda p=filepath:self.launch_file(p),bg=item_style['bg'],fg=item_style['fg'],font=("Arial",10),width=12).pack(side=tk.LEFT,fill=tk.X,expand=True)
        ...
        def task():
            ...
            try:
                ...
                if self.active_selenium_driver and (not driver_instance or driver_instance == self.active_selenium_driver):
                    self._terminate_selenium_driver_global()
                if target_config: target_config['process_info'] = None
                # Fallback may not be ideal here if it's already in a sequence.
    ...
    def launch_file(self, filepath, is_part_of_sequence=False):
        ...
        if is_part_of_sequence:
            ...
            # For Selenium: _launch_selenium_html_in_thread handles closing the *previous* Selenium window
            # if the new file is also HTML or if a non-HTML file is launched after an HTML file.
            # If current is not HTML, and previous was HTML, close Selenium
        ...
        try:
            if ext == '.py':
                ...
                elif os.uname().sysname == 'Darwin': popen_obj = subprocess.Popen([py_exe, abs_path])
                else: # Linux
                ...
                else: # Fallback if Selenium not available
                    webbrowser.open(f'file:///{abs_path}')
                    if current_config: current_config['process_info'] = None # No process to manage
    ...
    def _terminate_selenium_driver_global(self):
        """Safely quits the single global active Selenium driver and clears associated info."""
        if self.active_selenium_driver:
            ...
            self.active_selenium_driver = None # Clear global reference first
            self.active_selenium_filepath = None
            ...
            try:
                ...
                print(f"Error quitting global Selenium driver: {e}")
            
            # Clear process_info from the config it was associated with
            if filepath_it_was_for:
                ...
                if old_config and old_config.get('process_info') and \
                   old_config['process_info'].get('instance') == driver_to_quit: # Ensure it's the same instance
    ...
    def _terminate_process_info(self, filepath_to_terminate):
        ...
        if ptype == 'popen' and instance:
            try:
                instance.terminate()
                instance.wait(timeout=0.2) # Shorter timeout
            ...
            # If the instance to terminate is the currently active global selenium driver, use the global termination method.
            if instance and instance == self.active_selenium_driver:
                self._terminate_selenium_driver_global()
            elif instance : # Should not happen if logic is correct (only one active selenium driver)
                ...
                try: instance.quit()
                except Exception as e: print(f"Error quitting stray selenium instance: {e}")
        
        config_to_terminate['process_info'] = None # Clear info from config


    def run_selected_files_handler(self):
        # (Largely same as previous, but warning message is updated for Selenium)
        if self.is_running_sequence:
            self.is_running_sequence = False
            time.sleep(0.1) # Give sequence a moment to see the flag
             # Terminate all Popen processes and the single active Selenium driver
            for config_item in self.selected_file_configs:
                if config_item.get('process_info') and config_item['process_info']['type'] == 'popen':
                    self._terminate_process_info(config_item['path'])
            self._terminate_selenium_driver_global() # Terminate the selenium driver
        ...
        # Updated Warning based on what can be auto-closed
        ...
        def launch_sequence(index=0):
            ...
            if order == "random" and index == 0: # Shuffle only at the beginning of a pass
                random.shuffle(current_list)
            
            actual_index = index % len(current_list) # For looping

            if not current_list: # Should not happen if check at start, but safeguard
                self.is_running_sequence = False
                return
            ...
            if next_index < len(current_list) or loop:
                if not self.is_running_sequence: return # Check flag again before scheduling next
                
                # Add a small delay even for 0s to allow Selenium thread to kick off
                # and previous actions (like driver.quit) to potentially complete.
                actual_delay_ms = int(delay_seconds * 1000) if delay_seconds > 0 else 50 # Min 50ms for UI / thread sync
                
                self.root.after(actual_delay_ms, lambda: launch_sequence(next_index))
            else: # Reached end, not looping
                self.is_running_sequence = False
                print("Finished sequence.")
                # Optionally, after the very last item, if it was Selenium, close it after a final delay
                # if config_to_run['path'].lower().endswith(('.html','.htm')) and SELENIUM_AVAILABLE:
                #     final_close_delay = int(delay_seconds * 1000) if delay_seconds > 0 else 500
                #     self.root.after(final_close_delay, self._terminate_selenium_driver_global)
    ...
    def prompt_and_save_menu(self):
        # (Identical to previous, just ensure _generate_menu_script_content includes Selenium)
    ...
    def _generate_menu_script_content(self, menu_title_str, file_configs_with_styles, root_bg_str, num_cols_int):
        # This is crucial: The generated script must also have Selenium awareness.
        # The WhimsicalFileLauncherApp class definition pasted into the generated script
        # must be the *updated one* that includes all Selenium handling methods
        # and the SELENIUM_AVAILABLE check.
...
try:
    ...
    SELENIUM_AVAILABLE = False
    # print("Generated Menu: Selenium not found. HTML files will use default browser.") # Optional print
"""
        # The full class definition of WhimsicalFileLauncherApp (this entire class)
        # needs to be captured and put into the f-string.
        # For brevity, the placeholder method is used, but it implies pasting the whole class.
        # Ensure the pasted class uses the `selenium_imports_for_generated_script`'s SELENIUM_AVAILABLE.

        # --- Dynamically get the class source ---
        import inspect
        class_source = inspect.getsource(WhimsicalFileLauncherApp)
        # We need to make sure that the __main__ block of the original script is NOT part of the class source
        # and that methods like prompt_and_save_menu, _generate_menu_script_content, select_files_and_create_menu
        # are effectively disabled or not called when is_generated_menu is True (which current class logic does).

        # --- Construct the script ---
        script = f"""\
import tkinter as tk
from tkinter import filedialog, messagebox, simpledialog # Keep all for potential future use
import os
import subprocess
import webbrowser
import random
...
if __name__ == "__main__":
    ...
    if not SELENIUM_AVAILABLE: # Optional: Show warning in generated menu at startup
        # root_warn_gen = tk.Tk(); root_warn_gen.withdraw()
        # messagebox.showwarning("Selenium Missing", 
        #                        "Selenium library not found. HTML files will open in default browser.\\n"
        #                        "For fullscreen HTML viewing, please install Selenium and ChromeDriver.", 
        #                        parent=root_warn_gen)
        # root_warn_gen.destroy()
        pass # Console message is already handled by the import try-except

    root = tk.Tk()
    app = WhimsicalFileLauncherApp(
        root,
        is_generated_menu=True, # This is key
