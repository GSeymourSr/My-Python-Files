...
def crop_image_grid(image, vertical_lines, horizontal_lines, output_folder):
    ...
    for i in range(len(x_positions)-1):
        for j in range(len(y_positions)-1):
            ...
            output_path = os.path.join(output_folder, f"crop_{j}_{i}.png")
...
def save_cropped_images(event):
    ...
    if output_folder:
        crop_image_grid(image, vertical_lines, horizontal_lines, output_folder)
...
if __name__ == "__main__":
    ...
    # (Optional: Convert image to RGB if needed: image = image.convert("RGB"))
    ...
    # Show the image and set the origin to 'upper' so that (0,0) is the top‐left
    # (matching PIL’s coordinate system).
    ...
    save_button = Button(ax_button, 'Save Cropped Images')
