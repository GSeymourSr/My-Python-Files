...
# ==========================================
# 2. VISUAL ENGINE (Calculates the frames)
# ==========================================
class SpriteObj:
    ...
    def __init__(self, image, config: RenderConfig):
        ...
        # Start off-screen and aim for center-ish
...
class RenderEngine:
    def __init__(self, config: RenderConfig, bg_files, fg_files):
        ...
        self.bg_images = self._load_images(bg_files, is_bg=True)
        self.fg_images = self._load_images(fg_files, is_bg=False)
    ...
    def _load_images(self, file_paths, is_bg):
        imgs = []
        for path in file_paths:
            try:
                img = pygame.image.load(path).convert_alpha()
                if is_bg:
                    # Scale backgrounds to fit screen exactly
    ...
    def get_frame(self, t):
        ...
        # 1. Background Logic
        if self.bg_images and (t - self.last_bg_switch >= self.config.bg_interval):
        ...
        # 4. Return Numpy Array for MoviePy
...
class MovieMakerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Greg Seymour AI Movie Maker Pro v2.0")
    ...
    def _build_assets_tab(self):
        ...
        self.bg_mgr = self._create_asset_column(frame, 0, "Background Images", ('.jpg', '.png', '.jpeg'))
    ...
    def _create_asset_column(self, parent, col, title, exts):
        ...
        ttk.Button(btn_frame, text="+ Files", command=lambda: mgr.add_files(exts)).pack(side='left', expand=True, fill='x')
    ...
    def render(self):
        ...
        if not self.bg_mgr.files and not self.fg_mgr.files:
            messagebox.showerror("Error", "Please add at least some images in the Assets tab.")
            return
